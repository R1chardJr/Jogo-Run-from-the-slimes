...
] + [Platform(WIDTH // 2, FLOOR_Y + 32, base=True)] 
...
def update(dt):
    ...
    if game_state == "playing":
        ...
        if enemy_removal_timer >= 10 and len(enemies) > 2:
            enemies.pop(0)
        ...
        if enemy_spawn_timer >= enemy_spawn_interval and len(enemies) < MAX_ENEMIES:
            ...
            spawn_type = random.randint(0, 1)  # 0 = chão, 1 = plataforma

            if spawn_type == 0:
                # Spawn no chão
                while True:
                    spawn_x = random.randint(0, WIDTH - 48)
                    if abs(spawn_x - player.actor.x) >= 10:
                        break
                spawn_y = FLOOR_Y + 4
                patrol_width = 72  # Movimento livre no chão

            else:
                # Spawn em uma plataforma
                platform_centers = [153, 333, 513, 693]  # Centros das plataformas
                while True:
                    spawn_x = random.choice(platform_centers)
                    if abs(spawn_x - player.actor.x) >= 10:
                        break
                spawn_y = HEIGHT - 265   
                patrol_width = 36

            enemies.append(Enemy(spawn_x, spawn_y, patrol_width))
